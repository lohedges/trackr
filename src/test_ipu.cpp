...
poplar::Device setIpuDeviceWithIpuModelFallBack();
...
int main()
{
    ...
    MatrixRowMajorXf hit(num_planes, 2);
    ...
    std::vector<MatrixRowMajorXf> smoothed_hits_ref;

    MatrixRowMajorXf smooth_hit(4, num_hits);
    smooth_hit <<  0.01222589,  0.28513798,  0.19511343,  0.07082304,
                  -0.03856441,  0.16282753,  0.07284389,  0.05570415,
                  -0.15350827, -0.17637944,  0.2551298,   0.07082304,
                  -0.11140831, -0.16282753,  0.13283298,  0.05570415;
    smoothed_hits_ref.push_back(smooth_hit);

    smooth_hit << -0.02633852,  0.44796551,  0.26795733,  0.1265272,
                  -0.03856441,  0.16282753,  0.07284389,  0.05570415,
                  -0.26491657, -0.33920696,  0.38796278,  0.1265272,
                  -0.11140831, -0.16282753,  0.13283298,  0.05570415;
    smoothed_hits_ref.push_back(smooth_hit);

    smooth_hit << -0.06490293,  0.61079304,  0.34080122,  0.18223135,
                  -0.03856441,  0.16282753,  0.07284389,  0.05570415,
                  -0.37632488, -0.50203449,  0.52079576,  0.18223135,
                  -0.11140831, -0.16282753,  0.13283298,  0.05570415;
    smoothed_hits_ref.push_back(smooth_hit);

    smooth_hit << -0.10346735,  0.77362056,  0.41364511,  0.2379355,
                  -0.03856441,  0.16282753,  0.07284389,  0.05570415,
                  -0.48773319, -0.66486202,  0.65362875,  0.2379355,
                  -0.11140831, -0.16282753,  0.13283298,  0.05570415;
    smoothed_hits_ref.push_back(smooth_hit);

    smooth_hit << -0.14203176,  0.93644809,  0.48648901,  0.29363966,
                  -0.03856441,  0.16282753,  0.07284389,  0.05570415,
                  -0.59914149, -0.82768954,  0.78646173,  0.29363966,
                  -0.11140831, -0.16282753,  0.13283298,  0.05570415;
    smoothed_hits_ref.push_back(smooth_hit);
